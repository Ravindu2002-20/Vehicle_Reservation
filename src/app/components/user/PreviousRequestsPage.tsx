...
export function PreviousRequestsPage() {
  ...
  async function handleDelete(id: number) {
    const ok = globalThis.confirm("Delete this request?");
    if (!ok) return;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(payload?.error || "Could not delete request");
      }
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (s.includes("approved") || s === "allocated")
      return (
  ...
  const filteredRequests = requests.filter((req) => {
    ...
    const matchesStatus =
      statusFilter === "all"
        ...
        ? s.includes("approved") || s === "allocated"
        : s === "rejected";
            ...
            {isOpen && (
              <CardContent className="border-t bg-gray-50/50 p-4 space-y-4">
                <div className="pt-2">
                  ...
                  <p className="text-gray-800 font-medium">{new Date(req.created_at).toLocaleString()}</p>
                </div>
                ...
                {req.approval_status?.toUpperCase() === "REJECTED" && req.rejection_reason && (
                  <div className="text-orange-600 mt-2">{req.rejection_reason}</div>
                )}
